...
export default class Home extends React.Component {
  constructor() {
   ...
   openTx(txid){
     Linking.openURL('https://blockbook-qbc.service-crypto.com/tx/' + txid)
   }
   async sendReceive(txid){
     var address = await SecureStore.getItemAsync('address')
     return fetch('https://blockbook-qbc.service-crypto.com/api/v1/tx/' + txid)
   ...
   async balance(){
     ...
     if (address !== null){
       ...
       return fetch('https://blockbook-qbc.service-crypto.com/api/v1/address/' + address)
       .then((response) => response.json())
       .then((responseJson) => {
